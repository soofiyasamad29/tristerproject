/* global tristerthemeBgColors, tristerthemePreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( tristerthemeBgColors ).forEach( function( context ) {
					tristerthemeGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function tristerthemeGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'tristertheme-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#tristertheme-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( tristerthemePreviewEls[ context ] ) {
				_.each( tristerthemePreviewEls[ context ], function( items, setting ) {
	...
	$( function() {
		tristerthemeGenerateColorA11yPreviewStyles( 'content' );
		tristerthemeGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
