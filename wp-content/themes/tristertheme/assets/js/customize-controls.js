/* global tristerthemeBgColors, tristerthemeColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( tristerthemeBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( tristerthemeBgColors[ context ].color ) {
						backgroundColorValue = tristerthemeBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( tristerthemeBgColors[ context ].setting ).get();
					}
					tristerthemeSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( tristerthemeBgColors ).forEach( function( context ) {
			wp.customize( tristerthemeBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					tristerthemeSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		tristerthemeSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				tristerthemeSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function tristerthemeSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = tristerthemeColor( backgroundColor, accentHue );
	 ...
	 *
	 * @since Trister Theme 1.3
	 *
	 * @param {boolean} visible The visible value.
	 *
	 * @return {void}
	 */
	function tristerthemeSetRetineLogoVisibility( visible ) {
